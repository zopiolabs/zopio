...
interface PrismaModel {
  ...
  findUnique: (args: { where: Record<string, unknown> }) => Promise<Record<
    string,
    unknown
  > | null>;
  ...
  delete: (args: { where: Record<string, unknown> }) => Promise<
    Record<string, unknown>
  >;
}
...
export function createPrismaProvider(
  config: PrismaProviderConfig
): CrudProvider {
  const { client, modelMapping = {} } = config;
  ...
  const buildPagination = (pagination?: { page: number; perPage: number }): {
