...
export function createNotionProvider(
  ...
  const { apiKey, databaseMapping, propertyMapping = {} } = config;
    ...
    switch (type) {
      ...
      case 'formula': {
        ...
        return extractPropertyValue({
          type: formula.type,
          [formula.type]: formula[formula.type],
        } as NotionProperty);
      }
      case 'relation':
  ...
  const transformDataToProperties = (
  ...
  const transformWithMapping = (
    ...
    for (const [fieldName, propertyName] of Object.entries(mapping)) {
      if (data[fieldName] !== undefined) {
        properties[propertyName] = createPropertyValue(
          fieldName,
          data[fieldName]
        );
      }
  ...
  const addSortAndPagination = (
    ...
    if (sort) {
      const propertyName =
        propertyMapping[resource]?.[sort.field] || sort.field;
  ...
  const fetchAndProcessData = async (
    ...
    const data = result.results.map((page: NotionPage) =>
      transformPageToData(page, resource)
    );
  ...
  return {
    async getList({
      ...
      try {
        ...
        const { data, total } = await fetchAndProcessData(
          databaseId,
          resource,
          body
        );
    ...
    async create({ resource, variables }: CreateParams): Promise<CreateResult> {
      try {
        ...
        const properties = transformDataToProperties(
