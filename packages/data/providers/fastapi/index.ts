 ...
 */
export function createFastAPIProvider(
  config: FastAPIProviderConfig
): CrudProvider {
  const { apiUrl, token, customHeaders = {}, resources = {} } = config;
  ...
  function handleSimpleArrayResponse(result: any[]): {
    data: any[];
    total: number;
  } {
    return {
  ...
  function handlePaginatedResponse(result: any): {
    data: any[];
    total: number;
  } {
    return {
    ...
    async deleteOne({ resource, id }: DeleteParams): Promise<DeleteResult> {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}));
          throw new Error(
            `Failed to delete ${resource}/${id}: ${response.statusText} - ${JSON.stringify(errorData)}`
          );
        }
