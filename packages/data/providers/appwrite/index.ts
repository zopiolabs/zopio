 ...
 */
export function createAppwriteProvider(
  ...
  function buildAppwriteQueries(
    filter?: Record<string, unknown>,
    sort?: { field: string; order: string }
  ): string[] {
    return [...buildFilterParams(filter), ...buildSortParams(sort)];
  }
      ...
      try {
        ...
        const response = await fetch(buildUrl({ resource }), {
          ...
          body: JSON.stringify({
            ...
            documentId:
              (variables as Record<string, unknown>).$id || 'unique()',
          }),
        });
