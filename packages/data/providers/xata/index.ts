...
export function createXataProvider(config: XataProviderConfig): CrudProvider {
  const { apiKey, databaseURL, branch = 'main', tableMapping = {} } = config;
  ...
  const buildSortObject = (sort?: { field: string; order: string }): Record<
    string,
    string
  > | null => {
    if (!sort) {
    ...
    async create({ resource, variables }: CreateParams): Promise<CreateResult> {
      try {
        ...
        const response = await fetch(
          buildUrl(resource).replace('/query', '/data'),
          {
            method: 'POST',
            headers,
