...
export function createGoogleSheetsProvider(
  ...
  const { apiKey, spreadsheetId, sheetMapping = {} } = config;
  ...
  return {
    ...
    async create({ resource, variables }: CreateParams): Promise<CreateResult> {
      try {
        ...
        if (!headersResponse.ok) {
          throw new Error(
            `Failed to fetch headers: ${headersResponse.statusText}`
          );
        }
        ...
        const id =
          (variables as Record<string, unknown>).id?.toString() ||
          Date.now().toString();
    ...
    async update({
      ...
      id,
      variables,
    }: UpdateParams<Record<string, unknown>>): Promise<
