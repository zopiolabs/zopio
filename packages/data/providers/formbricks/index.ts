 ...
 */
export function createFormbricksProvider(
  config: FormbricksProviderConfig
): CrudProvider {
  const { apiUrl, apiKey, environmentId, resourceMapping = {} } = config;
  ...
  const buildUrl = (resource: string, id?: string | number): string => {
    ...
    const urlWithEnv =
      formbricksResource === 'surveys' || formbricksResource === 'responses'
  ...
  function buildFormbricksListUrl(
    resource: string,
  ...
  async function processFormbricksListResponse(
    response: Response
