...
export function createLocalProvider(
  ...
  const { storageType = 'localStorage', prefix = 'zopio_data_' } = config;
  ...
  return {
    async getList({
      ...
      sort,
      filter,
    }: GetListParams): Promise<GetListResult<unknown>> {
      try {
    ...
    async getOne({
      resource,
      id,
    }: GetOneParams): Promise<GetOneResult<unknown>> {
      try {
        // Simulate minimal async operation to satisfy linter
        await Promise.resolve();
    ...
    async create({
      resource,
      variables,
    }: CreateParams<unknown>): Promise<CreateResult<unknown>> {
      try {
        // Simulate minimal async operation to satisfy linter
        await Promise.resolve();
    ...
    async update({
      ...
      id,
      variables,
    }: UpdateParams<unknown>): Promise<UpdateResult<unknown>> {
      try {
        // Simulate minimal async operation to satisfy linter
        await Promise.resolve();
    ...
    async deleteOne({
      resource,
      id,
    }: DeleteParams): Promise<DeleteResult<unknown>> {
      try {
        // Simulate minimal async operation to satisfy linter
        await Promise.resolve();
