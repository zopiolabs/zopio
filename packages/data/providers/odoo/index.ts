...
export function createOdooProvider(config: OdooProviderConfig): CrudProvider {
  const { url, db, username, password, modelMapping = {} } = config;
  ...
  return {
    async getList({
      ...
      try {
        ...
        // Get data
        const rawData = await callModel<Record<string, unknown>[]>(
          model,
          'search_read',
          [domain],
          kwargs
    ...
    async getOne({ resource, id }: GetOneParams): Promise<GetOneResult> {
      try {
        ...
        // Get data
        const results = await callModel<Record<string, unknown>[]>(
          model,
          'search_read',
          [domain],
          { limit: 1 }
    ...
    async create({ resource, variables }: CreateParams): Promise<CreateResult> {
      try {
        ...
        const data = await callModel<Record<string, unknown>[]>(model, 'read', [
          [recordId],
        ]);
