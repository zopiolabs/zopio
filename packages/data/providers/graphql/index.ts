...
export function createGraphQLProvider(
  ...
  const { url, headers = {}, queryMapping = {}, resourceMapping = {} } = config;
  ...
  const processCustomQueryResult = (
    data: Record<string, unknown>,
    resource: string
  ): GetListResult => {
    ...
    return {
      data:
        result.items || result.data || (Array.isArray(result) ? result : []),
  ...
  return {
    async getList({
      ...
      try {
        ...
        const customResult = await executeCustomQuery(
          resource,
          pagination,
          sort,
          filter
      ...
      try {
        ...
        if (customQuery) {
          ...
          return {
            data:
              data[`create${type}`] ||
              data[`create${type.charAt(0).toUpperCase() + type.slice(1)}`] ||
              data,
          };
        }
        ...
        return {
          data: data[`create${type.charAt(0).toUpperCase() + type.slice(1)}`],
        };
      } catch (error) {
        throw error instanceof Error ? error : new Error(String(error));
      }
      ...
      try {
        ...
        if (customQuery) {
          ...
          return {
            data:
              data[`update${type}`] ||
              data[`update${type.charAt(0).toUpperCase() + type.slice(1)}`] ||
              data,
          };
        }
        ...
        return {
          data: data[`update${type.charAt(0).toUpperCase() + type.slice(1)}`],
        };
      } catch (error) {
        throw error instanceof Error ? error : new Error(String(error));
      }
