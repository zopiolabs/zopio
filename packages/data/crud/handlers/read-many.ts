...
function handlePagination(
  ...
  const isValid =
    !Number.isNaN(page) && page >= 1 && !Number.isNaN(limit) && limit >= 1;
...
function buildOrderBy<T>(queryParams: QueryParams): OrderBy<T> {
  ...
  if (queryParams.sortBy) {
    ...
    orderBy[queryParams.sortBy as keyof T] =
...
async function handleReadManyRequest<T extends BaseModel, R = T[]>(
  ...
  const { page, limit, skip, isValid } = handlePagination(
    queryParams,
    defaultLimit,
    maxLimit
  );
  ...
  const { totalPages, hasNextPage, hasPreviousPage } = createPaginationMetadata(
    page,
    limit,
    totalCount
  );
 ...
 */
export function readManyHandler<T extends BaseModel, R = T[]>(
  options: ReadManyHandlerOptions<T, R>
) {
