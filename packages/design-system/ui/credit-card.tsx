...
const PaymentIcons = {
  Visa: () => (
    <svg viewBox="0 0 40 24" className="w-full h-full">
      ...
      <text x="20" y="15" textAnchor="middle" className="text-xs font-bold fill-blue-600">
        VISA
      </text>
    </svg>
  ),
  ...
  "American Express": () => (
    <svg viewBox="0 0 40 24" className="w-full h-full">
      <rect width="40" height="24" rx="4" fill="white" />
      <text x="20" y="15" textAnchor="middle" className="text-xs font-bold fill-blue-700">
        AMEX
      </text>
    </svg>
  ),
    ...
    <CreditCardFlipContext.Provider value={true}>
      <div
        ...
        className={cn(
          ...
          supportsHover && "group-hover/kibo-credit-card:-rotate-y-180 group-hover/kibo-credit-card:shadow-lg",
...
export const CreditCardServiceProvider = ({
  ...
  return (
    <div className={cn("absolute right-0 bottom-0", "max-h-1/3 max-w-1/3", className)} {...props}>
      <IconComponent />
    </div>
  )
}
