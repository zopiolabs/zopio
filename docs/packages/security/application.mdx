...
Arcjet is configured in Zopio with two main features: bot detection and the Arcjet Shield WAF:
...
Zopio includes a boilerplate setup for Arcjet that protects against common threats to SaaS applications, but since the rules are defined in code, you can easily adjust them dynamically at runtime.
