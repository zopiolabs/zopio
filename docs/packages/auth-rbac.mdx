...
The `@zopio/auth-rbac` package provides Role-Based Access Control (RBAC) utilities using CASL, integrated with Clerk authentication. It enables fine-grained permission control based on user roles stored in Clerk's metadata.
