...
`Zopio` uses Neon as the database provider with Prisma as the ORM as well as Clerk for authentication. This guide will provide the steps you need to switch the database provider from Neon to EdgeDB.
...
Here's how to switch from Neon to [EdgeDB](https://edgedb.com) for your `Zopio` project.
...
You can also delete the now unused Prisma Studio app located at `apps/dbviewer`:
